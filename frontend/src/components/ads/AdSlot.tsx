...
// Popunder component — loads the full ExoClick PopMagic v8 script on mount.
// The script attaches its own click listener and handles frequency capping internally.
export function PopunderAd() {
  const siteSettings = useSiteSettings();
  const injected = useRef(false);

  useEffect(() => {
    const zoneId = siteSettings.ads.popunder?.zone_id?.trim();
    const network = siteSettings.ads.network;

    if (!siteSettings.ads.popunder?.enabled || !zoneId) return;
    if (injected.current) return;
    injected.current = true;

    if (network === "juicyads") {
      // JuicyAds: open URL directly on first click
    ...
    if (network === "exoclick" && /^\d+$/.test(zoneId)) {
      // ExoClick PopMagic v8 — must be loaded on mount (NOT on click).
      // The script self-attaches a click listener and manages its own capping.
      const script = document.createElement("script");
