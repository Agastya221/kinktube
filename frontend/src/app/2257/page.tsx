...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: settings.legal.compliance_2257.title,
    description: `${settings.branding.site_name} ${settings.legal.compliance_2257.title}`,
    robots: { index: true, follow: true },
  };
}

export default async function Page2257() {
  const settings = await getPublicSiteSettingsServer().catch(() => fallbackPublicSiteSettings);
  const doc = settings.legal.compliance_2257;
